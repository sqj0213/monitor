...
define("FDFSHTD_LOG_PATH_TMPL", "/data/site_log/storage/fastdfs/_DATE_/fastdfs-storage-fdhtd.log");
define("FDFSSTORAGE_LOG_PATH_TMPL", "/data/site_log/storage/fastdfs/_DATE_/fastdfs-storage-G-storage.log");
define("FDFSTRACKERD_ERROR_LOG_PATH_TMPL", "/data/site_log/storage/fastdfs/_DATE_/fastdfs-storage-G-tracker.log");
...
class picUploadReadLog{
	static function readPicUploadServerLog( $dateTime, $currentDate, $currentHour, $interval,$hostName="",$timezoneDiff=28800 )
	{
		...
		{
			$lineTime = strtotime(substr( $line, strpos( $line, "::" ) -19, 19 ));
			if ( !Empty( $hostName ) && $hostName == substr( $line, strpos( $line, " " ) ) )
			{
				if ( abs($lineTime-$dateTimeInt) <= $interval )
				{
					...
					{
						$line = "<font color=red><b>".$line."</b></b></font>";
	...
	{
		global $gearmanWorkerSlowCount,$gearmanWorkerCount;
		...
		while( $line = fgets($fileHandler) )
		{
			...
			if ( !Empty( $hostName ) && $hostName == substr( $line, strpos( $line, " " ) ) )
			{
				...
				{
					//if ( strpos( $line, "[fatal]" ) !== false )
					if ( floatval(substr($line, strpos( $line, "runTime:" )+8)) > 0.5 )
					{
						$line = str_replace("runTime","<font color=blue><b>runTime</b></b></font>",$line);
						if ( strpos( $line, "recive protocaldata" ) === false )
						{
							$msgBody .= $line;
