...
grep "\-_IP_"  ./taskTmpl/localhost-browser-jz-monitor.cfg|awk '{print $2}'|uniq|awk 'BEGIN{FS="-_IP_-"}{if ( $0!="") print $2"::"$0}' >>./taskTmpl/localhost-servicegroup.cfg
...
cat ./ip/hz-web.txt  |awk 'BEGIN{FS=":"}{print "sed -s s/_IP_/"$1"/g ./taskTmpl/localhost-browser-jz-monitor.cfg|sed -s s/_PORT_/"$2"/g   >> ./taskConf/localhost-hz-browser-jz.cfg"}'|sh
...
cat ./ip/ts-web.txt  |awk 'BEGIN{FS=":"}{print "sed -s s/_IP_/"$1"/g ./taskTmpl/localhost-browser-jz-monitor.cfg|sed -s s/_PORT_/"$2"/g   >> ./taskConf/localhost-ts-browser-jz.cfg"}'|sh
